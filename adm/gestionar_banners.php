...
// Conexión a la DB para obtener los banners existentes
require_once("script/conex_mysqli.php");
$cn = new MySQLcn();
$cn->query("SELECT idBanner, Titulo, Imagen, estado FROM banner ORDER BY fecha DESC");
$banners_existentes = $cn->fetchAll();
$cn->close();
?>
<!DOCTYPE html>
<html lang="es">
...
<body>
    <aside class="sidebar">
        ...
        <nav class="nav flex-column">
            ...
            <a class="nav-link active" href="gestionar_banners.php"><i class="fas fa-images fa-fw"></i> Banners</a>
            <a class="nav-link" href="#"><i class="far fa-newspaper fa-fw"></i> Noticias</a>
    ...
    <main class="main-content">
        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-lg-5">
                    <div class="card p-4">
                        <h4 class="mb-4">Añadir Nuevo Banner</h4>
                        <form action="procesar_imagen.php" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-check form-switch mb-4"><input class="form-check-input" type="checkbox" id="estado" name="estado" checked><label class="form-check-label" for="estado">Activar al subir</label></div>
                            <button type="submit" class="btn-submit w-100 p-2">Subir Banner</button>
                        </form>
                    </div>
                </div>

                <!-- Tabla de banners existentes -->
                <div class="col-lg-7">
                    <div class="card p-4">
                        <h4 class="mb-4">Banners Actuales</h4>
                        <div class="table-responsive">
                            <table class="table table-hover align-middle">
                                <thead><tr><th>Imagen</th><th>Título</th><th>Estado</th><th>Acciones</th></tr></thead>
                                <tbody>
                                    <?php foreach($banners_existentes as $banner): ?>
                                    <tr>
                                        <td><img src="../images/banner/<?php echo htmlspecialchars($banner['Imagen']); ?>" width="100" class="rounded"></td>
                                        <td><?php echo htmlspecialchars($banner['Titulo']); ?></td>
                                        <td><span class="badge bg-<?php echo $banner['estado'] ? 'success' : 'secondary'; ?>"><?php echo $banner['estado'] ? 'Activo' : 'Inactivo'; ?></span></td>
                                        <td>
                                    ...
                                    <?php endforeach; ?>
                                    <?php if(empty($banners_existentes)): ?>
                                    <tr><td colspan="4" class="text-center">No hay banners para mostrar.</td></tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
